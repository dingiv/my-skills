...
| [持久化状态管理](references/operations/persistence.md) | **数据建模（A）+ 存储格式（B）+ 持久化模式（C）+ 加载（D）+ 保存时机（E）+ 内存磁盘同步（F）+ 原子性与崩溃恢复（G）；文件型持久化，不覆盖 DB** | **引入新数据实体 / 写加载保存逻辑 / 设计同步策略 / 排查崩溃丢失——持久化全过程** |
