...
| --- | --- | --- |
| `code-master`（本 skill） | 项目层方法论：需求 → 接口（入口）→ 链路 → 依赖 → 空间 | 全部四子步 |
| `calltree` | 链路层描述：用 calltree 表达一段代码在干什么 | 步骤 2.2 后半（为接口画实现 calltree） |
| `rust-style` | Rust 代码层风格（含 OCP / trait / 生命周期 / 错误语义） | 步骤 2.4 之后 + 编码阶段；SOLID 中 O/D 见 rust-style 模块层 R1-R7 |
| `ts-style` | TypeScript 代码层风格（含 OCP / interface / 依赖倒置） | 步骤 2.4 之后 + 编码阶段；SOLID 中 O 见 ts-style 规则 1-4，D 见规则 3/6/9 |
...
- **rust-style 模块层 R1-R7**：Rust 实现（trait 契约、拆 trait、生命周期、依赖注入）
- **ts-style 规则 1-4 / 9**：TypeScript 实现（interface composition、工厂函数、显式依赖）
